    ...
    try {
        await new Promise((resolve, reject) => {
            const timeoutId = setTimeout(() => {
                reject(new Error(`Job timed out after ${TIMEOUT_MS / 1000} seconds`));
            }, TIMEOUT_MS);
        ...
        // The promise resolves with videoUrl if successful
        // We need to return it, but the wrapper function needs to handle the return
        // Since we are inside the try block of the wrapper function, we just need to adapt.
        
        // Wait, the structure above returns a Promise that resolves with videoUrl. 
